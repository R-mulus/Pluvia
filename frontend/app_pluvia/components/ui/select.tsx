...
function SelectValue({
  ...
  return (
    <SelectPrimitive.Value
      ref={ref}
      className={cn(
        "text-foreground line-clamp-1 flex flex-row items-center gap-2 text-sm font-outfit-medium",
    ...
    >
      ...
      {open ? (
        <Icon
          as={ChevronUp}
          aria-hidden={true}
          className="text-muted-foreground size-4 font-outfit"
        />
      ) : (
        <Icon
          as={ChevronDown}
          ...
          <TextClassContext.Provider value="text-popover-foreground">
            ...
            >
              <SelectPrimitive.Content
                className={cn(
                  "bg-popover border-border relative z-50 w-auto rounded-md border shadow-md shadow-black/5",
                  Platform.select({
                    web: cn(
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      className={cn(
        "text-muted-foreground px-2 py-2 text-xs sm:py-1.5 font-outfit",
        className,
      )}
      {...props}
    />
  ...
  return (
    <SelectPrimitive.Item
      className={cn(
        ...
        Platform.select({
          web: "focus:bg-accent focus:text-accent-foreground *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2 cursor-default outline-none data-[disabled]:pointer-events-none [&_svg]:pointer-events-none font-outfit",
        }),
        props.disabled && "opacity-50",
        className,
      )}
    ...
    >
      <View className="absolute right-2 flex size-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <Icon
            as={Check}
            className="text-muted-foreground size-4 shrink-0 font-outfit"
          />
        </SelectPrimitive.ItemIndicator>
      </View>
      <SelectPrimitive.ItemText className="text-foreground group-active:text-accent-foreground select-none text-sm font-outfit-medium" />
    </SelectPrimitive.Item>
  );
}
