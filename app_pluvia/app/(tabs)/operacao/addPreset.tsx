      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40, flexGrow: 1 }}>
          <View className="flex-row justify-between mb-6">
            <Header title={"Nova Predefinição"} subtitle="Biblioteca de Comandos" />
          ...
          <View className="gap-6 flex-1">
            <View className="gap-1">
              <Text className="text-xs text-subtexto font-outfit">Nome da Predefinição (Ex: Irrigação Noturna)</Text>
              <Controller
                control={control}
                name="nome"
            ...
            <View className="flex-row justify-between gap-4">
              ...
              <View className="items-start gap-1 flex-1">
                <Text className="text-xs text-subtexto font-outfit">Início (°)</Text>
                <Controller
                  control={control}
                  name="angulo_inicial"
                  render={({ field: { onChange, value } }) => (
              ...
              <View className="items-start gap-1 flex-1">
                <Text className="text-xs text-subtexto font-outfit">Final (°)</Text>
                <Controller
                  control={control}
                  name="angulo_final"
                  render={({ field: { onChange, value } }) => (
            ...
            <View className="gap-2">
              <Text className="text-sm font-outfit text-texto">Aplicar Água (Irrigação)</Text>
              <View className="flex-row items-center gap-3">
                <Text className={`text-base ${!isIrrigating ? "font-outfit-bold text-texto" : "text-subtexto"}`}>Não</Text>
                <Switch checked={isIrrigating} onCheckedChange={(val) => setValue("irrigacao", val)} />
                <Text className={`text-base ${isIrrigating ? "font-outfit-bold text-texto" : "text-subtexto"}`}>Sim</Text>
              </View>
            </View>
            ...
            <View className="items-center gap-6">
              <ToggleGroup
                value={direcao}
                onValueChange={(val) => val && setValue("direcao", val as "HORARIO" | "ANTI_HORARIO")}
                variant="outline"
                type="single"
              ...
              >
                <ToggleGroupItem
                  value="ANTI_HORARIO"
                  className={`flex-1 flex-row items-center justify-center gap-2 border-[2px] rounded-l-2xl border-primaria-azul h-12 ${direcao === "ANTI_HORARIO" ? "bg-primaria-azul" : "bg-transparent"}`}
                >
                  <RotateCcw size={20} strokeWidth={2.5} color={direcao === "ANTI_HORARIO" ? "white" : "#00A0A6"} />
                  <Text className={`text-base font-outfit-medium ${direcao === "ANTI_HORARIO" ? "text-white" : "text-primaria-azul"}`}>Reverso</Text>
                </ToggleGroupItem>

                <ToggleGroupItem
                  value="HORARIO"
                  className={`flex-1 flex-row items-center justify-center gap-2 border-[2px] rounded-r-2xl border-primaria-azul h-12 ${direcao === "HORARIO" ? "bg-primaria-azul" : "bg-transparent"}`}
                >
                  <Text className={`text-base font-outfit-medium ${direcao === "HORARIO" ? "text-white" : "text-primaria-azul"}`}>Horário</Text>
                  <RotateCw size={20} strokeWidth={2.5} color={direcao === "HORARIO" ? "white" : "#00A0A6"} />
                </ToggleGroupItem>
              </ToggleGroup>

              <View className="flex-row items-center w-full gap-4 mt-6">
                <Button className="bg-incorreto rounded-pluvia flex-1 h-[45px]" onPress={() => router.back()}>
                  <Text className="text-white font-outfit-bold">Cancelar</Text>
                </Button>
                <Button className="bg-primaria-verde rounded-pluvia flex-1 h-[45px]" onPress={handleSubmit(onSubmit)} disabled={isPending}>
                  {isPending ? <ActivityIndicator color="white" /> : <Text className="text-white font-outfit-bold">Salvar na Biblioteca</Text>}
                </Button>
              </View>
            </View>
          </View>
